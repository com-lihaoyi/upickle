...
object shared {
  object that {
    ...
    object That {
      implicit def rw: upickle.default.ReadWriter[That] =
  ...
  object other {
    ...
    object Other {
      implicit def rw: upickle.default.ReadWriter[Other] =
        upickle.default.macroRW
    }
  }
  object common {
    ...
    object Message {
      implicit def rw: upickle.default.ReadWriter[Message] =
        upickle.default.macroRW
    }
  }
}
...
object All {
  ...
  sealed trait Outers
  object Outers {
    implicit def rw: upickle.default.ReadWriter[Outers] =
  ...
  object Inners {
    implicit def rw: upickle.default.ReadWriter[Inners] =
      upickle.default.ReadWriter.merge(
        Inner1.rw,
        Inner2.rw
      )
  ...
  object Inner1 {
    implicit def rw: upickle.default.ReadWriter[Inner1] =
      upickle.default.macroRW
  }
  case class Inner2(a: Message) extends Inners
  object Inner2 {
    implicit def rw: upickle.default.ReadWriter[Inner2] =
      upickle.default.macroRW
  }
}
...
sealed trait Gadt[T]
object Gadt {
  implicit def rw[T]: ReadWriter[Gadt[T]] =
    macroRW[Gadt[_]].asInstanceOf[ReadWriter[Gadt[T]]]
...
//        test - rw(A(Seq("1", "2", "3")), """{"t":["1","2","3"]}""")
      ...
      test("ADT") {
        ...
        test - {
          ...
          rw(
            D1.Insert(1, 1),
            s"""{"$$type":"$pref1.Insert","key":1,"value":1}"""
          )
          rw(
            D1.Insert(1, 1): D1[Int, Int],
            s"""{"$$type":"$pref1.Insert","key":1,"value":1}"""
          )
          rw(D1.Remove(1), s"""{"$$type":"$pref1.Remove","key":1}""")
          rw(
            D1.Remove(1): D1[Int, Int],
            s"""{"$$type":"$pref1.Remove","key":1}"""
          )
        ...
        test - {
          ...
          rw(
            D2.Insert(1, 1),
            s"""{"$$type":"$pref2.Insert","key":1,"value":1}"""
          )
          rw(
            D2.Insert(1, 1): D2[Int, Int],
            s"""{"$$type":"$pref2.Insert","key":1,"value":1}"""
          )
          rw(D2.Remove(1), s"""{"$$type":"$pref2.Remove","key":1}""")
          rw(
            D2.Remove(1): D2[Int, Int],
            s"""{"$$type":"$pref2.Remove","key":1}"""
          )
    ...
    test("recursiveDataTypes") {
      ...
      rw(
        SingleNode(
          123,
          List(SingleNode(456, Nil), SingleNode(789, Nil))
        ): SingleTree,
        """{
          "$type": "upickle.Recursive.SingleNode",
          "value": 123,
...
//        test - rw(Gadt2.CopyOver(Seq(1, 2, 3), ""): Gadt2[_, Unit], """{"$type":"upickle.Gadt2.CopyOver","src":[1,2,3],"v":""}""")
    ...
    test("issues") {
      ...
      test("hash-collision") {
        def zeroHashCodeStrings: Iterator[String] = {
          def charAndHash(h: Int): Iterator[(Char, Int)] =
            ('!' to '~').iterator.map(ch => (ch, (h + ch) * 31))

          for {
            (ch0, h0) <- charAndHash(0)
            (ch1, h1) <- charAndHash(h0)
            (ch2, h2) <- charAndHash(h1)
            if ((h2 + 32) * 923521 ^ (h2 + 127) * 923521) < 0
            (ch3, h3) <- charAndHash(h2)
            if ((h3 + 32) * 29791 ^ (h3 + 127) * 29791) < 0
            (ch4, h4) <- charAndHash(h3)
            if ((h4 + 32) * 961 ^ (h4 + 127) * 961) < 0
            (ch5, h5) <- charAndHash(h4)
            if ((h5 + 32) * 31 ^ (h5 + 127) * 31) < 0
  ...
  class Thing[T: upickle.default.Writer, V: upickle.default.Writer](
      t: Option[(V, T)]
  ) {
