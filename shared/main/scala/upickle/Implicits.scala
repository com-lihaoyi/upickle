...
object Implicits extends Implicits with Generated{

  protected[this] def validate[T](name: String)(pf: PartialFunction[Js.Value, T]): PartialFunction[Js.Value, T] = Internal.validate(name)(pf)

}

trait LPM extends Macros
/**
 * Typeclasses to allow read/writing of all the common
 * data-types and data-structures in the standard library
 */
trait Implicits extends Types with LPM{
  import Internal._
  import Aliases._
